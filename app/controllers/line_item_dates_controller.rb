class LineItemDatesController < ApplicationController
  ...
  def update
    if @line_item_date.update(line_item_date_params)
      respond_to do |format|
        ...
        # We would have been fine to not need the `app/views/line_item_dates/update.turbo_stream.erb` file,
        # but since we want the extra logic to make sure the dates are all in chrono order, we have to
        # point to that `app/views/line_item_dates/update.turbo_stream.erb` to access that logic.
  ...
  def destroy
    # This action appears to work in the browser w/o adding the hotwire, but the page is actually
    # reloading, which naturally excludes the recently-deleted record. However, we don't want to
    # reload the page, so we need to expand into a `format.turbo_stream` format and corresponding view.
